...
function loadDirectPriceShadowDOMP() {
	...
	<div class="modal-container"> <button class="close" id="close-modal">&times;</button>
		<div class="modal-header"> <img class="cd-logo-b" src="${extension_icon}" alt="${extension_name}" />
			<div class="modal-title" id="exampleModalLabel"><span class="plus">+</span> Thanks for earning eGift Card Rewards with us. Remember to be on the lookout for plugin notifications while you are shopping on Google Search & Shopping tabs. Learn more about how to earn and redeem here: <a href="https://www.commercedirect.io">commercedirect.io</a></div>
		</div>
...
</div>`, shadowRootPopup.innerHTML += ` <div class="modal-wrap" id="modal-reward-amount">
...
</div> `, shadowRootPopup.innerHTML += ` <div class="modal-wrap" id="modal-reward">
    ...
    `, appendPopupStyleInline(shadowRootPopup), chrome.storage.local.get("userid", function(e) {
		void 0 !== e.userid && (user_id = "to Commerce Direct", shadowRootPopup.getElementById("user").innerText = user_id, shadowRootPopup.getElementById("user1").innerText = user_id, shadowRootPopup.getElementById("user2").innerText = user_id)
			...
			chrome.runtime.sendMessage({
				...
				if ("Y" == (e = JSON.parse(e)).body.qualified) {
					...
					var t = shadowRootPopup.getElementById("rewardlink"),
						...
						a = shadowRootPopup.getElementById("modal-reward"),
						n = shadowRootPopup.getElementById("redeem_link");
					t.addEventListener("click", function() {
						s.classList.toggle("visible")
					}), d.addEventListener("click", function() {
						s.classList.remove("visible")
					});
					var i = shadowRootPopup.getElementById("next-modal"),
					...
					i.addEventListener("click", function() {
						...
						shadowRootPopup.getElementById("vcard").innerHTML = e + " Points", chrome.runtime.sendMessage({
							...
							"Y" == (o = JSON.parse(o)).body.qualified ? (chrome.storage.local.set({
								asked_amt: e
							}, function() {}), shadowRootPopup.getElementById("pts_validation_msg").innerText = "", a.classList.toggle("visible")) : (shadowRootPopup.getElementById("pts_validation_msg").innerText = o.body.message, setTimeout(function() {
