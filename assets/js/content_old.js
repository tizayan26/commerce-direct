...
function loadLoginShadowDOM(){
  ...
  '<div class="login-close" id="login_close">X</div></div>';
...
function loadDirectPriceShadowDOM(obj){
  ...
  appendStyleInlineSearch(shadowRoot);
...
function appendStyleInlineSearch(node){
...
function getClientID(){
  if(window.location.hostname == "www.google.com"){
    ...
    const client_id = (matches[0]=== undefined || matches[0] === null) ? matches[0].split('=') : matches[0].trim().split('=');
    return client_id[1];
  }else{
